...
$sql = $koneksi_master->query("SELECT * FROM pb_master.tb_barang a INNER JOIN pb_master.tb_satuan c ON a.kode_satuan=c.kode_satuan WHERE a.STATUS='A' AND id_jenis_barang='$id_jenis_barang' and not exists (SELECT b.kode_barang FROM pb_transaksi.tb_permintaan_detail b WHERE b.kode_barang=a.id_barang AND b.status='A' AND b.kode_permintaan='$kode_permintaan') ORDER BY a.nama_barang ASC");

while ($databarang = $sql->fetch_assoc()) {

?>
<!-- <option value="<?php echo $databarang["id_barang"] ?>"><?php echo $databarang["nama_barang"] ?></option><br> -->
