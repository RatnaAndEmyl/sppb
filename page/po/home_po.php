            ...
            <div class="col-12">
                <div class="card card-primary">
                    <div class="card-header">
                        <h3 class="card-title">DAFTAR PURCHASE ORDER
                            <!-- <?php echo $_SESSION['s_pilih_tanggal'] . $_SESSION['s_status_po']; ?> (<?php echo $_SESSION['s_pilih_pengguna_po'] ?>) (<?php echo $_SESSION['s_pilih_departemen_po'] ?>) -->
                            <?php echo $_SESSION['s_pilih_tanggal'] ?>
                        </h3>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            $sqltext = "SELECT a.kode_po, a.tanggal_po, a.username, a.jabatan, b.nama_gudang, a.status_po, c.nik, d.nama_departemen FROM pb_transaksi.tb_po a INNER JOIN pb_master.tb_gudang b ON a.kode_gudang=b.kode_gudang INNER JOIN pb_role.tb_user c ON a.nik=c.nik INNER JOIN pb_master.tb_departemen_karyawan d ON c.kode_departemen=d.kode_departemen";
                            ...
                            if ($level == 'SUPERVISOR') {
                                $sqltext = $sqltext . "and exists (SELECT x.* from pb_transaksi.tb_po_detail x where x.kode_po=a.kode_po) and c.kode_departemen='$kode_departemen' ";
                            }

                            $sqltext = $sqltext . " AND '" .  $_SESSION['s_pilih_gudang'] . "' = b.nama_gudang
                             AND d.nama_departemen = '$_SESSION[s_pilih_departemen]'
                             AND a.tanggal_po between '$tgl_awal' and '$tgl_akhir'";

                            // $sqltext = $sqltext . " AND '" .  $_SESSION['s_pilih_tanggal'] . "' = date_format (a.tanggal_po,'%Y-%m')
                            ...
                            if ($_SESSION['s_status_permintaan'] <> 'ALL') {
                                $sqltext = $sqltext . " AND '" . $_SESSION['s_status_permintaan'] . "' = a.status_po";
                            }
                            ...
                            while ($data = $sql->fetch_assoc()) {
                                $status_po = '-';
                                if ($data['status_po'] == 'A') {
                                    $status_po = 'Perlu Persetujuan' and $color = 'bg-warning';
                                    ...
                                    $sql_count_status_selain_a = $koneksi_master->query("SELECT COUNT(kode_po) 'jumlah' FROM pb_transaksi.tb_po_detail WHERE status='A' AND status_po<>'A' AND kode_po='" . $data['kode_po'] . "'");
                                    $tampil_count_status_selain_a = $sql_count_status_selain_a->fetch_assoc();
                                } elseif ($data['status_po'] == 'Y') {
                                    $status_po = 'Sudah Disetujui' and $color = 'bg-success';
                                    $sql_count_status = $koneksi_master->query("SELECT COUNT(kode_po) 'jumlah' FROM pb_transaksi.tb_po_detail WHERE status='A' AND status_po='Y' AND kode_po='" . $data['kode_po'] . "'");
                                    $tampil_count_status = $sql_count_status->fetch_assoc();
                                    ...
                                    $status_po = 'Ditolak!' and $color = 'bg-danger';
                                    $sql_count_status = $koneksi_master->query("SELECT COUNT(kode_po) 'jumlah' FROM pb_transaksi.tb_po_detail WHERE status='A' AND status_po='N' AND kode_po='" . $data['kode_po'] . "'");
                                    $tampil_count_status = $sql_count_status->fetch_assoc();
                            ...
                            ?>
                                ...
                                <div class="card col-sm-12 shadow p-2 mb-4 bg-body" style="padding: 4px; background-color:bg-white; border-style: solid; border-width: thin; border-top-left-radius: 50px; border-bottom-right-radius: 50px; background-color: 	#F5F5F5;">
                                    <div class="card-header text-dark text-center" style="border-width: thin; border-top-left-radius: 40px; border-bottom-right-radius: 20px; background-color: <?php echo $bg_color ?>">
                                        <h6><strong><?php echo strtoupper(strftime("%A, %d %B %Y", strtotime($data['tanggal_po']))); ?></strong></h6>
                                                    ...
                                                    <div class="col-md-6">
                                                        <div class="form-group row">
                                                            ...
                                                            <div class="col-md-6">
                                                                ...
                                                                <a href="?page=po&aksi=detail&kode_po=<?php echo $data['kode_po']; ?>&pc=<?php echo $angka; ?>&oc=<?php echo md5($data['kode_po'] . $angka); ?>" class="btn btn-success" title="Lihat Detail"><i class="fas fa-eye"></i></a>

                                                                <?php if (($level == 'USER') or ($level == 'ADMIN')) {
                                                                    if ($tampil_count_status_selain_a['jumlah'] == 0) {
                                                                        if (($data['status_po'] == 'A') and ($_SESSION['s_nik'] == $data['nik'])) { ?>
                                                                            <a onclick="return confirm('Apakah Anda Yakin Ingin Menghapus ke- <?php echo $tampil_count_status['jumlah'] ?> Barang Ini?')" href="?page=po&aksi=hapus&kode_po=<?php echo $data['kode_po']; ?>&pc=<?php echo $angka; ?>&oc=<?php echo md5($data['kode_po'] . $angka); ?>" class="btn btn-danger" title="Hapus Data"><i class="fas fa-trash"></i></a>
