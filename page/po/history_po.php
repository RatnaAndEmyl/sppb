            ...
            <div class="col-12">
                <div class="card card-primary">
                    <div class="card-header">
                        <h4 class="card-title">HISTORY PO BARANG</h4>
                    </div>
                    <div class="card-body">
                        <a href="?page=po" class="btn btn-dark" style="margin-bottom: 5px; "><i class="fa fa-undo"></i> Kembali</a>

                        <a href="?page=po&aksi=home_po" class="btn btn-secondary" style="margin-bottom: 5px; "><i class="fas fa-home"></i> Home po</a>
                                    ...
                                    <table id="example1" class="table table-bordered table-hover">
                                        <thead>
                                            <tr>
                                                ...
                                                <th scope="col" style="text-align:center; vertical-align:middle;">Status PO</th>
                                        ...
                                        <tbody>
                                            ...
                                            <?php
                                            $no = 1;
                                            $TextQuery = "SELECT a.kode_po, a.tanggal_po, c.nama, a.jabatan, b.nama_gudang, a.status_po, d.nama_suplier FROM pb_transaksi.tb_po a 
                                            INNER JOIN pb_master.tb_gudang b ON a.kode_gudang=b.kode_gudang
                                            INNER JOIN pb_role.tb_user c ON a.kode_user=c.kode_user INNER JOIN pb_master.tb_suplier d ON a.kode_suplier=d.kode_suplier INNER JOIN (SELECT x.kode_po, COUNT(x.kode_po) AS 'jumlah' FROM pb_transaksi.tb_po_detail x WHERE x.status <> 'N' GROUP BY x.kode_po ORDER BY x.kode_po)xx ON a.kode_po = xx.kode_po WHERE a.status='A' AND b.status='A' AND c.status='A' AND d.status='A' AND xx.jumlah > 0";


                                            // INNER JOIN (SELECT x.kode_po, COUNT(x.kode_po) AS 'jumlah' FROM pb_transaksi.tb_po_detail x WHERE x.status <> 'N' GROUP BY x.kode_po ORDER BY x.kode_po)xx ON a.kode_po = xx.kode_po WHERE a.status='A' AND xx.jumlah > 0";
                                            ...
                                            while ($data = $sql->fetch_assoc()) {
                                                ...
                                                $status_po = '-';
                                                if ($data['status_po'] == 'A') {
                                                    $status_po = 'Perlu Persetujuan' and $colortext = 'text-warning';
                                                    ...
                                                    if ($tampil_count_status['jumlah'] <> 0) { ?>
                                                        <tr>
                                                            ...
                                                            <td style="text-align:center; font-weight: bold; vertical-align:middle;">
                                                                <?php echo $tampil_count['jumlah']; ?> - <?php echo $data['kode_po']; ?></td>
                                                            <td style="text-align:center; vertical-align:middle;">
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td style="text-align:center; vertical-align:middle;">
                                                                <?php echo $no++; ?></td>
                                                            <td style="text-align:center; vertical-align:middle;">
                                                                <?php echo $tampil_count_status['jumlah']; ?> - <?php echo $data['kode_po']; ?></td>
                                                            <td style="text-align:center; vertical-align:middle;">
                                                                <?php echo strftime("%d %B %Y", strtotime($data['tanggal_po'])); ?></td>
                                                            <td style="text-align:center; vertical-align:middle;">
                                                                <?php echo $data['nama']; ?> <br>
                                                            ...
                                                            <td style="text-align:center; vertical-align:middle;">
                                                                ...
                                                                <a href="?page=po&aksi=detail&kode_po=<?php echo $data['kode_po']; ?>&pc=<?php echo $angka; ?>&oc=<?php echo md5($data['kode_po'] . $angka); ?>" class="btn btn-success"><i class="fa-regular fa-eye"></i></a>

                                                                <?php
                                                                if ($tampil_count_status['jumlah'] == 0) {
                                                                    if ($level <> 'SUPERVISOR') { ?>
                                                                        <a onclick="return confirm('Apakah Anda Yakin untuk Menghapus Data Ini ?')" href="?page=po&aksi=hapus&kode_po=<?php echo $data['kode_po']; ?>&pc=<?php echo $angka; ?>&oc=<?php echo md5($data['kode_po'] . $angka); ?>" class="btn btn-danger"><i class="fas fa-trash"></i></a>
                                                                    <?php } ?>
                                                                <?php } ?>
