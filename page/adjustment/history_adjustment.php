            ...
            <div class="col-12">
                <div class="card card-primary">
                    <div class="card-header">
                        <h4 class="card-title">HISTORY ADJUSTMENT BARANG</h4> <br>
                        <?php echo $tampil_jumlah_permintaan['jumlah_permintaan']; ?>
                    </div>
                    <div class="card-body">

                        <a href="?page=adjustment" class="btn btn-dark" style="margin-bottom: 5px; "><i class="fa fa-undo"></i> Kembali</a>
                                ...
                                <table class="table table-striped table-bordered datatable-select-inputs">
                                    <table id="example1" class="table table-bordered table-hover">
                                        ...
                                        <tbody>
                                            ...
                                            <?php
                                            $no = 1;
                                            $TextQuery = "SELECT a.kode_adjustment, a.tanggal_adjustment, c.nama, a.jabatan, b.nama_gudang, a.status_adjustment, a.create_by FROM pb_transaksi.tb_adjustment a 
                                            ...
                                            if ($level == 'USER') {
                                                ...
                                                $TextQuery = "SELECT a.kode_adjustment, a.tanggal_adjustment, c.nama, a.jabatan, b.nama_gudang, a.status_adjustment, a.create_by FROM pb_transaksi.tb_adjustment a INNER JOIN pb_master.tb_gudang b ON a.kode_gudang=b.kode_gudang INNER JOIN pb_role.tb_user c ON a.nik=c.nik INNER JOIN (SELECT x.kode_adjustment, COUNT(x.kode_adjustment) AS 'jumlah' FROM pb_transaksi.tb_adjustment_detail x WHERE x.status <> 'N' GROUP BY x.kode_adjustment ORDER BY x.kode_adjustment)xx ON a.kode_adjustment = xx.kode_adjustment WHERE a.status='A' AND xx.jumlah > 0 AND c.kode_departemen='$kode_departemen'";
                                            }
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td style="text-align:center; vertical-align:middle;">
                                                                ...
                                                                <?php if (($tampil_count_status['jumlah'] == 0) and ($_SESSION['s_kode_user'] == $data['create_by'])) { ?>
                                                                    <a onclick="return confirm('Apakah anda Yakin Ingin Menghapus ke- <?php echo $tampil_count['jumlah']; ?> Barang Ini?')" href="?page=adjustment&aksi=hapus&kode_adjustment=<?php echo $data['kode_adjustment']; ?>&pc=<?php echo $angka; ?>&oc=<?php echo md5($data['kode_adjustment'] . $angka); ?>" class="btn btn-danger" title="Hapus Data"><i class="fas fa-trash"></i></a>
