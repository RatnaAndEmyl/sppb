...
$str_departemen = "select kode_departemen from hr_master.tb_departemen where kode_mitrakerja='$kode_mitrakerja' and status='A' ";
$sql_departemen = $koneksi_master->query($str_departemen);
$tampil_departemen = $sql_departemen->fetch_assoc();

$str_subdepartemen = "select * from hr_master.tb_subdepartemen where kode_mitrakerja='$kode_mitrakerja' and kode_departemen in (select kode_departemen from hr_master.tb_departemen where kode_mitrakerja='$kode_mitrakerja' and status='A') and status='A' ";
$sql_subdepartemen = $koneksi_master->query($str_subdepartemen);
$tampil_subdepartemen = $sql_subdepartemen->fetch_assoc();

$str_jabatan = "select kode_jabatan from hr_master.tb_jabatan where kode_mitrakerja='$kode_mitrakerja' and status='A' ";
...
$str_benefit = "select kode_benefit from hr_master.tb_benefit where kode_mitrakerja='$kode_mitrakerja' and status='A'";
$sql_benefit = $koneksi_master->query($str_benefit);
$tampil_benefit = $sql_benefit->fetch_assoc();


$str_tugas_tanggung_jawab = "select kode_tugas_tanggung_jawab from hr_master.tb_tugas_tanggung_jawab where kode_mitrakerja='$kode_mitrakerja' and status='A'";
...
$str_kriteria = "select kode_kriteria_nonset from hr_master.tb_kriteria where kode_mitrakerja='$kode_mitrakerja' and status='A'";
$sql_kriteria = $koneksi_master->query($str_kriteria);
$tampil_kriteria = $sql_kriteria->fetch_assoc();

$str_golongan = "select kode_golongan from hr_master.tb_golongan where kode_mitrakerja='$kode_mitrakerja' and status='A' ";
...
(select jenis_kartu from hr_transaksi_karir.tb_profile_kartu where nik='$nik' and status='A' and jenis_kartu in(select kode_trxtype from hr_master.tb_view_detail where kode_view='KV001') GROUP by jenis_kartu) x
";

// select count(nik) 'jumlah' from hr_transaksi_karir.tb_profile_kartu where nik='$nik' and status='A' and jenis_kartu in(select kode_trxtype from hr_master.tb_view_detail where kode_view='KV001')


$sql_kartu = $koneksi_master->query($str_kartu);
$tampil_kartu = $sql_kartu->fetch_assoc();

$str_jumlah_kartu = "select count(kode_trxtype) 'jumlah' from hr_master.tb_view_detail where kode_view='KV001' AND status='A' ";
$sql_jumlah_kartu = $koneksi_master->query($str_jumlah_kartu);
$tampil_jumlah_kartu = $sql_jumlah_kartu->fetch_assoc();

$str_keluarga_ayah = "select count(nik) 'jumlah' from hr_transaksi_karir.tb_profile_keluarga where nik='$nik' and status='A' and uraian in (select kode_trxtype from hr_master.tb_view_detail where kode_view='KV002') ";
$sql_keluarga_ayah = $koneksi_master->query($str_keluarga_ayah);
$tampil_keluarga_ayah = $sql_keluarga_ayah->fetch_assoc();

$str_jumlah_keluarga_ayah = "select count(kode_trxtype) 'jumlah' from hr_master.tb_view_detail where kode_view='KV002' ";
...
$str_keluarga_suami_istri = "select count(nik) 'jumlah' from hr_transaksi_karir.tb_profile_keluarga where nik='$nik' and status='A' and uraian in (select kode_trxtype from hr_master.tb_view_detail where kode_view='KV003') ";
...
$str_jumlah_keluarga_suami_istri = "select count(kode_trxtype) 'jumlah' from hr_master.tb_view_detail where kode_view='KV003' ";
                ...
                <div class="row">
                  <div class="col-md-12">
                    ...
                    <div class="timeline">
                     ...
                     hr_master.tb_progress_pelamar b on a.kode_progress=b.kode_progress join 
                     hr_master.tb_trxtype c on b.kode_trxtype=c.kode_trxtype where a.status='A' and nik='$nik' order by tgl_create  desc ");
              ...
              <div class="col-md-3" style=" padding-bottom: 10px;">
                <div class="input-group-prepend">
                  ...
                  <select class="custom-select" id="penjadwalan" name="penjadwalan" style="width: 200px;" required>
                    ...
                    $sql1 = $koneksi_master->query("SELECT * from hr_master.tb_progress_pelamar a join hr_master.tb_trxtype c on a.kode_trxtype=c.kode_trxtype where a.status='A' and exists(select b.kode_trxtype from hr_master.tb_view_detail b where b.kode_view='KV004' and  b.status='A' and a.kode_trxtype=b.kode_trxtype) order by kode_progress");
